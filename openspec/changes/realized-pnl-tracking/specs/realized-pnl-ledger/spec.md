...
系统 SHALL 在交易层与账户层同时存储了结盈亏，且金额单位为账户原币种。`transactions` MUST 新增 `realizedPnl` 字段（默认 `0`），`accounts` MUST 新增累计字段 `realizedPnl`（默认 `0`）。

#### Scenario: 新交易默认值

- **WHEN** 系统创建任意非卖出交易，或 `affectHolding=false` 的卖出交易
...
系统 MUST 将上线前历史交易的 `realizedPnl` 统一初始化为 `0`，且本次变更 MUST NOT 做历史反推回填。
