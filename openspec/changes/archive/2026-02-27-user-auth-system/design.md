...
**Step 2: Auth 基础设施** 4. 安装依赖：next-auth, @auth/drizzle-adapter, bcrypt 5. 配置 Auth.js（providers, callbacks, adapter）6. 创建 /login 和 /register 页面 7. 添加 middleware.ts 路由守卫

**Step 3: 数据迁移** 8. 创建迁移脚本：插入默认 admin 用户 9. 将现有无主数据的 userId 设为默认用户 10. userId 列改为 NOT NULL

**Step 4: API 改造** 11. 所有业务 API 加 session 检查 + userId 过滤 12. seed.ts 改为按用户执行

**Step 5: 前端适配** 13. layout.tsx 加 SessionProvider 14. navbar 显示用户信息 + 登出 15. 管理后台页面
