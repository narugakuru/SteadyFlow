...
历史实现曾为 Twelve Data 免费套餐规避限频而引入固定 65s 批次等待，该策略会显著拉长请求耗时。当前策略假设使用 Twelve Data 的用户为 Pro 套餐，不再需要固定秒级等待。  
...
- 选择：腾讯请求使用逗号拼接批量拉取，单批设置固定上限；当返回 `v_pv_none_match=\"1\"` 或批次缺失标的时，对缺失标的做有限次快速重试（无固定秒级等待）。
- 理由：实测可用但存在边缘风控返回，快速重试可降低误判失败率，同时避免人为延长整体请求时长。
- 备选方案：不重试直接失败。实现简单，但会把临时风控误判为供应商不可用，降低成功率。

### 4) 移除固定延时策略（含 Twelve Data 65s）

- 选择：报价链路（Tencent/EODHD/Twelve Data）不引入固定秒级 sleep；Twelve Data 仅保留最小必要重试与超时控制，不做批次间隔等待。
- 理由：用户已明确以 Pro 套餐为默认前提，固定 65s 等待会显著恶化交互体验，且已不再是必要限频手段。
- 备选方案：保留可配置延时参数。可控性更高，但默认路径复杂、易误配，且与“无延时设计”目标冲突。
...
- [腾讯批量返回可能出现缺项或 `none_match`] -> 批次内按缺失 symbol 做有限快速重试，超限后进入回退链路。
...
- [去除固定延时后短时请求密度上升] -> 通过批次上限、超时与失败快速重试上限控制峰值，不回退到秒级 sleep。
...
4. 回归测试：
   ...
   - US/JP 路由不回归。
5. 移除 Twelve Data 65s 相关常量与等待实现，确保全链路无固定秒级延时。
6. 发布后观察失败明细中 provider 分布与异常类型，必要时微调批量分片和重试参数。
