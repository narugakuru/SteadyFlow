...
- `historyProvider`：获取 ATH 计算所需的日线序列。
- `vixProvider`：获取 VIX 图表与最新可用收盘值。

推荐映射：

- Tencent 快照：A 股、港股及其核心指数。
- Stooq 快照：美股、日股和可直接稳定返回的全球指数。
- Stooq 历史：DAX、道琼斯、纳指、标普、日经等需要回溯历史高点的标的。
- CBOE 官方 CSV：VIX 日线历史与最新可用收盘值。

这样设计的原因是，`Stooq` 对 VIX 当前/历史可用性不稳定，而 CBOE 官方 CSV 能稳定提供 VIX 日线；Tencent 适合补足中国和港股市场的指数快照。页面消费的是统一规范化后的结果，不直接感知底层源差异。
...
- [VIX 无稳定实时值] → 使用 CBOE 最新可用日线收盘值驱动说明与图表，文案上避免暗示“实时逐笔”。
