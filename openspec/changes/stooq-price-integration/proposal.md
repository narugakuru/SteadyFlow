...
Yahoo Finance API（yahoo-finance2）已完全失效，市场概览页无法获取任何指数数据。同时，持仓股价目前只能手动更新，对于有明确股票代码的美股/日股持仓，应支持自动拉取最新价格。此外，现有 `cost` 字段在 shares 模式下语义模糊（存储总成本），需要重新定义为"平均每股成本"以支持更清晰的成本价/现价分离展示。

## What Changes

- **移除 yahoo-finance2 依赖**，替换为 Stooq 免费 CSV API 作为市场数据源
- **市场指数配置重写**：符号映射从 Yahoo 格式改为 Stooq 格式，仅保留有数据的 6 个指数（S&P 500、纳斯达克100、道琼斯、日经225、VIX、恒生指数），A 股 4 个 + 恒生科技 + 东证指数暂显示为无数据，留出扩展空间
- **新增持仓自动报价功能**：通过 Stooq API 批量拉取有合法 ticker（`.us`/`.jp` 后缀）的持仓现价，更新 `price` 和 `marketValue`
...
- **新建持仓表单增强**：ticker 字段增加格式提示，引导用户填写 Stooq 格式（如 `aapl.us`、`7203.jp`）
...
- `auto-quote-fetch`: 持仓自动报价功能，批量拉取有 Stooq 格式 ticker 的持仓现价并更新 price/marketValue

### Modified Capabilities

- `market-overview`: 数据源从 yahoo-finance2 改为 Stooq，指数配置重写，不支持的指数显示为无数据
...
- **依赖**：移除 `yahoo-finance2`，无新增 npm 依赖（Stooq 使用原生 fetch）
