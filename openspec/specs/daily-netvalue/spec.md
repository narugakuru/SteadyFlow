...
系统 SHALL 将每日净值快照的 `totalAssetCny` 作为收益率（TWR）计算的组合市值序列来源。收益率计算 MUST NOT 抓取或反推历史行情生成市值序列，且 MUST 容忍净值快照在自然日上不连续。

#### Scenario: 净值快照供给收益率计算

- **WHEN** 收益率端点需要组合市值序列
- **THEN** 系统读取当前用户净值快照中的 `totalAssetCny`

#### Scenario: 类现金资产纳入收益率市值

- **WHEN** 用户组合包含银行存款、支付宝余额等类现金资产
- **THEN** 这些资产已包含在每日总资产快照中，可直接参与收益率计算
