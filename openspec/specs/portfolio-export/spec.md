...
系统 SHALL 提供 `GET /api/export/portfolio` 端点，并支持通过请求参数切换导出详细度。`detail=full` SHALL 返回完整投资组合 JSON 快照；`detail=decision` SHALL 返回面向决策的精简快照。完整快照 MUST 在单次响应中包含当前用户的账户、持仓、资产类别、汇率、总资产摘要、资产配置派生结果以及报价同步元数据，避免调用方再拼装多个接口。精简快照 MUST 仅保留总资产盈亏摘要、各类资产占比与标的持仓等决策相关信息。

#### Scenario: 已登录用户获取完整快照

- **WHEN** 已登录用户请求 `GET /api/export/portfolio` 或 `GET /api/export/portfolio?detail=full`
- **THEN** 系统返回 200 JSON，并在同一响应中包含 `meta`、`summary`、`raw`、`derived` 四个顶层对象

#### Scenario: 已登录用户获取精简快照

- **WHEN** 已登录用户请求 `GET /api/export/portfolio?detail=decision`
- **THEN** 系统返回 200 JSON，并只包含决策所需的 `meta`、`summary` 与资产配置/标的持仓数据，不包含 `raw`、`accountBreakdown` 等低价值结构
...
- **WHEN** 用户存在 `marketValue=0` 的持仓
- **THEN** 无论请求完整快照还是精简快照，导出响应都不包含这些零市值标的
...
- **WHEN** 已登录用户请求 `GET /api/export/portfolio?download=1&detail=full`
- **THEN** 系统以 JSON 文件下载形式返回完整投资组合快照，并附带可识别的文件名

#### Scenario: 下载精简导出文件

- **WHEN** 已登录用户请求 `GET /api/export/portfolio?download=1&detail=decision`
- **THEN** 系统以 JSON 文件下载形式返回精简决策快照，并附带可识别的文件名

#### Scenario: 下载内容与接口内容一致

- **WHEN** 用户分别访问同一详细度下的导出接口与下载接口
