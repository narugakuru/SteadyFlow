...
系统 SHALL 在所有页面顶部显示统一的导航栏，包含以下导航项：总览(/)、市场(/market)、账户(/accounts)、交易(/transactions)、快照(/snapshots)、股价更新(/batch-update)。导航栏右侧 SHALL 显示当前登录用户信息和登出按钮。admin 用户 SHALL 额外显示"管理"导航项（/admin）。
