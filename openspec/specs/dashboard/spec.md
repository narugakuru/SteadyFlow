...
总览页 SHALL 在首屏核心区域展示 Wealthfolio 风格的绿色填充资产趋势图，并保持原有白色浅色主题与既有配色。该图表 MUST 使用现有净值历史图表数据作为资产价值趋势来源，并明确表示资产曲线而非历史盈亏、TWR、IRR 或投资业绩曲线。图表可使用稍深绿色线条与浅绿填充，但 MUST NOT 显示收益 0 水平线。图表下方 SHALL 提供时间范围切换控件，首版范围复用现有净值图表范围：`30d`、`90d`、`1y`、`3y`、`all`。
