...
系统 SHALL 在总览页提供绿色填充资产面积图，用于展示当前用户净值历史中的总资产走势。该图表 MUST 使用统一数值格式化函数展示金额，并在 Tooltip 中显示日期与总资产金额。该图表表示资产价值趋势，不表示历史盈亏、TWR、IRR 或投资业绩曲线。
...
系统 SHALL 为洞察页提供当前持仓热力图。热力图块大小 MUST 表示持仓市值权重，颜色 MUST 表示当前盈亏比例，Tooltip 或等价详情 MUST 展示持仓名称、账户、当前市值、盈亏金额与盈亏比例。
