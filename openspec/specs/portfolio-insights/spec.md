...
系统 SHALL 提供 `/insights` 洞察页，作为当前用户投资组合可视化分析入口。洞察页 MUST 只读取当前登录用户的数据，并在全局左侧边栏中以“洞察”导航项展示。页面 SHOULD 保持原有白色浅色主题与既有业务配色，避免引入深色视觉基调。
...
- **WHEN** 已登录用户打开洞察页
- **THEN** 页面沿用浅色卡片与白底视觉，不切换为深色主题

### Requirement: 洞察页组合占比图表

洞察页 SHALL 展示三个组合占比图表：货币占比、账户占比、资产类别占比。所有占比 MUST 以统一基准货币折算后的当前总资产为分母。图表 MUST 使用当前快照数据，不要求历史数据。图表配色 SHOULD 使用更清透、明亮的组合色，避免暗沉深色。
...
洞察页 SHALL 展示当前持仓热力图。热力图块面积 MUST 基于当前持仓市值折算值；颜色 MUST 基于当前持仓盈亏比例，并遵守用户 `colorMode` 设置。热力图颜色 SHOULD 使用柔和红/绿并按盈亏比例强度分成 2-3 档，避免重色块；热力图只表示当前持仓当前盈亏状态，不表示历史表现或每日变化。
