...
- 外部市场数据：持仓报价为美股 Yahoo Finance（yahoo-finance2，`quote` + `quoteSummary` 兜底，按盘前/盘后/常规交易状态选择当前价）优先、EODHD realtime 回退；A/港/北交所为腾讯简易行情优先、EODHD realtime 回退、Twelve Data 最低权重备份；EODHD 回退按最多 10 个 symbol 一组使用 realtime 批量请求且不使用 EOD/历史收盘价补价，Twelve Data 不使用 `previous_close` 作为成功报价；旧市场页聚合数据读取入口和 Stooq/CBOE 读取适配已移除。
...
src/
├── app/
│   ├── page.tsx                    # 总览页 Dashboard（资产曲线 + 纪律表 + 再平衡）
│   ├── layout.tsx                  # 根布局（AppSessionProvider + AppShell）
│   ├── insights/page.tsx           # 洞察页（货币/账户/资产类别占比 + 当前持仓热力图）
│   ├── accounts/page.tsx           # 账户管理页
│   ├── transactions/page.tsx       # 活动/交易记录页
│   ├── netvalue/page.tsx           # 净值历史页（图表 range+grain + 列表分页）
│   ├── batch-update/page.tsx       # 已下线，直接重定向到 /
│   ├── market/page.tsx             # 已下线，直接重定向到 /
│   ├── login/page.tsx              # 登录页
...
│       ├── transfers/              # 账户互转原子创建（双账户现金与 principal）
...
│   ├── vix-sentiment.tsx           # 已下线市场页旧 VIX 单态区间说明组件
...
│   ├── transaction-form.tsx        # 交易表单（共享组件，支持账户互转、预填与内联新建持仓）
...
│   │   ├── portfolio-insights-service.ts # 洞察页当前快照读模型
...
│   │   ├── utils.ts                # 工具函数
│   │   ├── account-principal.ts    # 账户原始资金、累计盈亏与费用损耗计算
│   │   ├── account-transfer.ts     # 账户互转金额校验、delta 与回滚计算
...
| accounts           | 投资账户       | userId(FK, not null), name, currency(CNY/USD/HKD), cashBalance(现金余额), principal(原始资金/净投入本金，账户原币种), realizedPnl(累计了结盈亏，账户原币种), sortOrder(账户默认排序)                                                                                                                                                                                                              |
| holdings           | 持仓明细       | accountId(FK), name, ticker, valuationMode(amount/shares), cost, marketValue, shares, price, assetClass, memo(持仓备注)                                                                                                                                                                                                                                                                           |
| transactions       | 交易记录       | accountId(FK), holdingId(FK可选), type(buy/sell/dividend/deposit/withdraw/fee/transfer_out/transfer_in), transferGroupId(互转组标识), counterpartyAccountId(对手账户), date, amount, realizedPnl(单笔了结盈亏，账户原币种), cashDelta, principalDelta, holdingSharesDelta, holdingCostDelta, holdingMarketValueDelta, shares, price, fee, affectCash(影响现金, 0/1), affectHolding(影响持仓, 0/1) |
